...
describe(`subscribe user tests`, () => {
  ...
  test(`Should send back 204 on success`, async () => {
    const response = await request(app).patch(`/platforms/${mockPlatform.title}/subscribe`);

    expect(mockPlatformModel.subscribers.indexOf(mockUser.username)).toBeGreaterThan(-1);
    expect(mockUserModel.subscriptions.indexOf(mockPlatform.title)).toBeGreaterThan(-1);
